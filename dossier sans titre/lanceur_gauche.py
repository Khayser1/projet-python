...
class projectile_gauche(pygame.sprite.Sprite):
    ...
    def __init__(self,player ): #on met comme argument la classe player importer les corrdonner du joueur 
        ...
        self.rect.x= player.rect.x+34 #on met comme position de spawn le joueur en x
    ...
    def supprimé(self):
        self.player.groupe_projectiles_gauche.remove(self)
        print("supprimé")
        
        
    def move_projectile_gauche(self):
        self.rect.x-=self.velocity
        self.rotation()
        
        
        for monstres in self.player.jeux.check_collision(self, self.player.jeux.groupe_monstre_gauche): 
            self.supprimé()
            monstres.dégats_gauche(self.player.attack)
        if self.rect.x<-1080:
            self.supprimé()
